...
def generator_batch(dataset,batchsize=32):
    ...
    while True:
        for batch in range(batchsize):
            random_data = dataset.iloc[[np.random.randint(len(dataset))]].reset_index()
...
def main(_):
    ...
    for i in range(1,FLAGS.epochs):
        ...
        model.fit_generator(training_gen,samples_per_epoch=len(X_train),nb_epoch=i,validation_data=X_validation, nb_val_samples=len(X_validation))
